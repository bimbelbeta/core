...
export function TargetSelectionDialog() {
	...
	const handleClose = () => {
		setOpen(false);
		saveToStorage(`target-dialog-dismissed-${userKey}`, new Date().toISOString());
	};

	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogContent showCloseButton={true} className="max-h-[85vh] overflow-y-auto">
				<button
					type="button"
					onClick={handleClose}
						...
						{(field) => (
							<div className="space-y-2">
								<Label htmlFor={field.name}>
									Universitas
									{isPending && <Spinner className="inline size-3" />}
								</Label>
								<Select
									value={field.state.value?.toString() ?? ""}
									onValueChange={(value) => {
										const prevUniversityId = field.state.value;
