...
const TIME_ELAPSED_BEFORE_SHOWING_AGAIN = 1000 * 60 * 60 * 24 * 7; // 7 days
...
export function TargetSelectionDialog() {
	...
	const { data: universities } = useQuery(orpc.university.list.queryOptions({ input: {} }));
	const { data: universityDetail } = useQuery({
		...orpc.university.find.queryOptions({ input: { id: form.state.values.universityId ?? 0 } }),
