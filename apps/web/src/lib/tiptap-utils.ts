...
// tiptap-ui-primitive components use BEM-style class names, not Tailwind utility overrides,
// so tailwind-merge is intentionally excluded here. If primitives ever receive Tailwind
// utility overrides from consumers, switch to importing cn from @/lib/utils instead.
...
/** Returns true if the named mark exists in the editor schema. */
export const isMarkInSchema = (markName: string, editor: Editor | null): boolean => {
	if (!editor?.schema) return false;
	return editor.schema.spec.marks.get(markName) !== undefined;
};

/** Returns true if the named node exists in the editor schema. */
...
/** Returns true if pos is a non-null, non-negative number. */
...
/**
 * Placeholder stub for image upload — currently simulates progress with a delay
 * and returns a hardcoded placeholder URL. Replace with a real upload implementation
 * (e.g., multipart POST to a storage service) before using in production.
